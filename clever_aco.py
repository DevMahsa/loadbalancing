...
def random_permutation(cities):
    perm = np.array(range(len(cities)))
    for i in perm:
...
def global_update_pheromone(phero, cand, decay):
    candidate = cand['vector']
    for i in range(0,len(candidate)):
        x = candidate[i]
        y = candidate[iif(i == len(candidate) - 1, 0, i + 1)]
...
def search(cities, max_it, num_ants, decay, c_heur, c_local_phero, c_greed):
    ...
    for iter in xrange(max_it):
        ...
        print(" > iteration %d, best=%s" % (iter + 1, best['cost']))
    return best
...
def main():
    ...
    print("Done. Best Solution: c=%s, v=%s" % (best['cost'], str(best['vector'])))
